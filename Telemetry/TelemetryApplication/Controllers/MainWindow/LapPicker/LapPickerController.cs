namespace SecondMonitor.Telemetry.TelemetryApplication.Controllers.MainWindow.LapPicker
{
    ...
    public class LapPickerController : ILapPickerController
    {
        ...
        private void AddLaps(IReadOnlyCollection<LapSummaryDto> lapsSummary)
        {
            ...
            LapSummaryDto bestSector1Lap = _loadedLaps.Where(x => x.Sector1Time > TimeSpan.Zero).OrderBy(x => x.Sector1Time).First();
            ...
            LapSummaryDto bestSector2Lap = _loadedLaps.Where(x => x.Sector2Time > TimeSpan.Zero).OrderBy(x => x.Sector1Time).First();
            ...
            LapSummaryDto bestSector3Lap = _loadedLaps.Where(x => x.Sector3Time > TimeSpan.Zero).OrderBy(x => x.Sector1Time).First();
